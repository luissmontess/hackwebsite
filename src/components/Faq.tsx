...
export default function Faq() {
    ...
    return (
        <section 
            id="faq" 
            className="min-h-screen flex flex-col items-center text-center px-4 sm:px-8 py-16"
        >
            <h2 className="text-3xl font-bold text-gray-800 mb-8 mt-10">FAQ</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl w-full">
                {faqs.map((faq, index) => (
                    <div key={index} className="w-full">
                        <button
                            onClick={() => toggleFaq(index)}
                            className="w-full bg-gray-100 text-left p-4 rounded-lg shadow-md flex justify-between items-center transition-all duration-300 hover:bg-gray-200"
                        >
                        ...
                        <div
                            className={`transition-all duration-500 overflow-hidden ${
                                openIndex === index ? "max-h-40 opacity-100 p-4 bg-white rounded-lg shadow-md mt-2" : "max-h-0 opacity-0"
                            }`}
                        >
